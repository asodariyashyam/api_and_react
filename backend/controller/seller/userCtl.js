...
module.exports.signin = async (req, res) => {
  try {
    ...
    db.query(sql, [email], async function (error, result) {
      ...
      const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, "userToken");
      res.cookie("token", token, { httpOnly: true });

      const data = {token , cookieData}; 
      return commonClass.reply(res, 200, false, "Sign In successfully !!", data);
    });
  } catch (error) {
